...
class SchemaMapper {
  ...
  normalizeSpec(spec) {
    ...
    if (Array.isArray(spec)) {
      ...
      if (spec.$schema) {
        let name = spec.$schema; // inject referenced schema spec
  ...
  map(data, callback, options) {
    ...
    if (SchemaMapper.specIsTransient(spec) && options && options.skipTransients) return; // We have to pass the data in as a object property as that is the only way to reference data

    return this.mapField(spec, 'root', {
      root: data
  ...
  mapRecursive(spec, object, callback, options = {}, meta = {}) {
    ...
    var specTemp = clone(spec); // If match all spec is defined, newSpec defaults to an empty object since any spec rules should be replaced by
