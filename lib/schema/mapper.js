...
class SchemaMapper {
    ...
    map(data, callback, options) {
        ...
        if (SchemaMapper.specIsTransient(spec) && options && options.skipTransients)
            return;
    ...
    mapPaths(paths, callback, options, meta) {
        ...
        objects.forEach((object) => {
            for (var fieldPath in object) {
                meta.path = fieldPath;
                var spec = util_1.default.getSpec(fieldPath, this.specNormalised);
    ...
    mapQueryPaths(query, callback, options = {}) {
        ...
        const mapRecursiveQuery = (query) => {
            if (type_caster_1.default.getType(query) == Object) {
                for (let fieldName in query) {
                    if (!query.hasOwnProperty(fieldName))
                        continue;
                    if (util_1.default.isQueryOperator(fieldName)) {
                        if (util_1.default.canValidateQueryOperator(fieldName)) {
                        ...
                        if (type_caster_1.default.getType(query[fieldName]) == Object) {
                            for (let childFieldName in query[fieldName]) {
                                hasOpertators = hasOpertators || util_1.default.isQueryOperator(childFieldName);
                                if (hasOpertators && util_1.default.canValidateQueryOperator(childFieldName)) {
    ...
    mapField(spec, fieldName, container, callback, options, meta = {}) {
        ...
        if (spec)
            fieldType = spec.constructor == String ? spec : type_caster_1.default.getType(spec);
        if (fieldType == Object && spec.$type !== undefined)
            fieldType = spec.$type;
        ...
        switch (fieldType) {
            case Object:
                if (spec.$spec !== undefined)
                    spec = spec.$spec;
                this.mapRecursive(spec, container ? container[fieldName] : undefined, callback, options, meta);
                break;
            case Array:
                ...
                else if (type_caster_1.default.getType(spec) == Object && spec.$spec) {
