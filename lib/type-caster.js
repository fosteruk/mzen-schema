...
class TypeCaster {
    ...
    static cast(toType, value) {
        ...
        if (fromTypeCaster == undefined)
            throw new Error('Can not cast from unknown type "' + fromTypeName + '"');
        const toTypeName = TypeCaster.getTypeName(toType);
        const toTypeCaster = TypeCaster.type[TypeCaster.getTypeName(toType)];
        if (toTypeCaster == undefined)
            throw new Error('Can not cast to unknown type "' + toTypeName + '"');
        var result = value;
...
class TypeCasterObjectID {
    static castString(value) {
        ...
        return new bson_objectid_1.default(value);
    }
}
TypeCaster.type = {
...
//# sourceMappingURL=type-caster.js.map