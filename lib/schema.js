...
class Schema {
  ...
  validateQuery(query, options) {
    ...
    options = options ? options : {}; // This is a query - we are expecting fields which are not defined
  ...
  filterPrivate(object, mode, mapperType) {
    ...
    this.mapper[mapperType](object, (fieldSpec, fieldName, fieldContainer, path) => {
      ...
      if (filters.private === true || filters.private == mode) {
        ...
        if (fieldContainer) deleteRefs.push({
          fieldContainer,
          fieldName
        });
      }
      ...
      if (filters.privateValue === true || filters.privateValue == mode) {
        ...
        if (fieldContainer) valueReplaceRefs.push({
          fieldContainer,
          fieldName
        });
      }
  ...
  specToFieldType(spec, value) {
    var fieldType = undefined; // If the field type is a string value then it should contain the string name of the required type (converted to a constructor later).
  ...
  async validateField(spec, fieldName, value, path, options, meta = {}) {
    ...
    var fieldType = this.specToFieldType(spec, value); // Configure default value filter if not already set
    ...
    if (defaultValue !== undefined) {
      value = await Filter.filter(value, {
        defaultValue
      });
    }
    ...
    if (fieldType == Object && (strict || options.strict)) {
      ...
      if (strict !== undefined) options.strict = strict; // In strict mode we must ensure there are no fields which are not defined by the spec
    ...
    value = await Filter.filter(value, filters); // notNull can be defaulted via global option
  ...
  typeCast(requiredType, value, path, meta = {}) {
    ...
    if (!skip) {
      ...
      var valueTypeName = TypeCaster.getTypeName(value); // We compare type names rather than constructors
      ...
      if (requiredTypeName != valueTypeName) {
        ...
        if ( // We failed to convert to the specified type
        resultTypeName != requiredTypeName || // We converted to type 'number' but the result was NaN so its invalid
  ...
  static isNull(value) {
    var result = value === null || // The string value NULL or null are treated as a literal null
