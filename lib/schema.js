...
class Schema {
    ...
    init() {
        if (!this.mapper) {
            this.mapper = new mapper_1.default(this.spec, this.config);
    ...
    applyTransients(object) {
        ...
        return (object && this.constructors) ? this.mapper.map(object, (fieldSpec, fieldName, fieldContainer, path, meta) => {
            if (fieldContainer) {
                ...
                if (pathRef) {
                    fieldContainer[fieldName] = object_path_accessor_1.default.getPath(pathRef, meta.root);
                }
    ...
    stripTransients(object) {
        this.init();
        return (object && this.constructors) ? this.mapper.map(object, (fieldSpec, fieldName, fieldContainer) => {
            if (fieldContainer) {
                var pathRef = fieldSpec ? fieldSpec.$pathRef : null;
                if (pathRef) {
    ...
    validate(object, options) {
        ...
        this.mapper.map(object, (fieldSpec, fieldName, fieldContainer, path) => {
            let promise = this.validateField(fieldSpec, fieldName, fieldContainer ? fieldContainer[fieldName] : undefined, path, options, meta).then((value) => {
                if (fieldContainer)
    ...
    filterPrivate(object, mode = true, mapperType = 'map') {
        this.init();
        mode = mode ? mode : true;
        var deleteRefs = [];
        var valueReplaceRefs = [];
        var mapperType = (mapperType == 'mapPaths') ? 'mapPaths' : 'map';
        this.mapper[mapperType](object, (fieldSpec, fieldName, fieldContainer) => {
        ...
        deleteRefs.forEach((ref) => {
            if (ref.fieldContainer && ref.fieldContainer[ref.fieldName])
                delete ref.fieldContainer[ref.fieldName];
        });
    }
    specToFieldType(spec, value) {
        ...
        if (spec) {
            if (spec.constructor == String) {
                ...
                fieldType = type_caster_1.default.getType(spec);
                if (fieldType === Object) {
        ...
        if (fieldType && fieldType.constructor == String) {
            ...
            fieldType = types_1.default[fieldType];
        }
        return fieldType;
    }
    validateField(spec, fieldName, value, path, options, meta = {}) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            options = options ? options : {};
            if (!util_1.default.isValidFieldName(fieldName)) {
            ...
            else if (fieldName == '_id' && fieldType == types_1.default.ObjectID) {
                defaultValue = function () {
                    return new types_1.default.ObjectID;
                };
            }
            ...
            if (defaultValue !== undefined) {
                value = yield filter_1.default.filter(value, { defaultValue });
            }
            if (value != undefined) {
                ...
                if (fieldType && fieldType != types_1.default.Mixed)
            ...
            // Apply filters
            value = yield filter_1.default.filter(value, filters);
            ...
            var validateResults = yield validator_1.default.validate(value, validators, { name, root: meta.root });
    ...
    typeCast(requiredType, value, path, meta = {}) {
        ...
        if (!skip) {
            var result = value;
            var requiredTypeName = type_caster_1.default.getTypeName(requiredType);
            var valueTypeName = type_caster_1.default.getTypeName(value);
            ...
            if (requiredTypeName != valueTypeName) {
                result = type_caster_1.default.cast(requiredType, value);
                let resultTypeName = type_caster_1.default.getTypeName(result);
                if (
    ...
    static mergeValidationResults(results) {
        ...
        results.forEach(function (result) {
            if (result.errors)
                Object.assign(finalResult.errors, result.errors);
        });
